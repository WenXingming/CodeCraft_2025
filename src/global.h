...
vector<Tag> tags;
// 按 read 量进行排序，read 多的放在磁盘前面。因为后续每个标签的分配区间满了的话，需要从磁盘的后向前插入对象。为了根据 tagId 快速找到所属的 tag 对象，需要维护一个 hash 表。f(x): tagId 为 x 的对象，其所属 tag 对象在 tags 中的下标。注：这里 tagIdToTagsIndex(M+1) 没用！因为 M 还未确定呢
